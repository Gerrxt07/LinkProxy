...
import com.linkpowered.api.event.proxy.ListenerBoundEvent;
import com.linkpowered.api.event.proxy.ListenerCloseEvent;
...
import com.linkpowered.proxy.network.netty.SeparatePoolInetNameResolver;
import com.linkpowered.proxy.protocol.netty.GameSpyQueryHandler;
...
/**
 * Manages endpoints managed by Link, along with initializing the Netty event loop group.
 */
public final class ConnectionManager {
  ...
  public void logChannelInformation() {
    ...
    Natives.compress.getFallbackReason()
        .ifPresent(reason -> LOGGER.warn("Native compression unavailable; using Java fallback. {}",
            reason));
    Natives.cipher.getFallbackReason()
        .ifPresent(reason -> LOGGER.warn("Native cipher unavailable; using Java fallback. {}",
            reason));
  }
  ...
  /**
   * Creates a TCP {@link Bootstrap} using Link's event loops.
   *

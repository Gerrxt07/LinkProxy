...
import com.linkpowered.proxy.protocol.packet.LegacyPlayerListItemPacket.Item;
 ...
 */
public class LinkTabListLegacy extends KeyedLinkTabList {

  private final Map<String, UUID> nameMapping = new ConcurrentHashMap<>();

  public LinkTabListLegacy(final ConnectedPlayer player, final ProxyServer proxyServer) {
    ...
    switch (packet.getAction()) {
      case LegacyPlayerListItemPacket.ADD_PLAYER:
        if (nameMapping.containsKey(item.getName())) { // ADD_PLAYER also used for updating ping
          KeyedLinkTabListEntry entry = entries.get(nameMapping.get(item.getName()));
          if (entry != null) {
          ...
          entries.put(uuid, (KeyedLinkTabListEntry) TabListEntry.builder()
              .tabList(this)
  ...
  public TabListEntry buildEntry(GameProfile profile,
    ...
    return new LinkTabListEntryLegacy(this, profile, displayName, latency, gameMode);
  }

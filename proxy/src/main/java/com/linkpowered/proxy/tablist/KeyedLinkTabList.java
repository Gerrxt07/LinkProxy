...
package com.linkpowered.proxy.tablist;
...
import com.linkpowered.api.proxy.player.ChatSession;
import com.linkpowered.api.proxy.player.TabListEntry;
...
import com.linkpowered.proxy.protocol.packet.chat.RemoteChatSession;
 ...
 */
public class KeyedLinkTabList implements InternalTabList {

  protected final ConnectedPlayer player;
  protected final MinecraftConnection connection;
  protected final ProxyServer proxyServer;
  protected final Map<UUID, KeyedLinkTabListEntry> entries = new ConcurrentHashMap<>();

  /**
   * Creates a new LinkTabList.
   */
  public KeyedLinkTabList(final ConnectedPlayer player, final ProxyServer proxyServer) {
    this.player = player;
  ...
  public void addEntry(TabListEntry entry) {
    ...
    Preconditions.checkArgument(entry instanceof KeyedLinkTabListEntry,
        "Not a Link tab list entry");
    ...
    entries.put(entry.getProfile().getId(), (KeyedLinkTabListEntry) entry);
  }
  ...
  public void clearAll() {
    Collection<KeyedLinkTabListEntry> listEntries = entries.values();
  ...
  public TabListEntry buildEntry(GameProfile profile,
    ...
    return new KeyedLinkTabListEntry(this, profile, displayName, latency, gameMode, key);
  }

  @Override
  public TabListEntry buildEntry(GameProfile profile, @Nullable Component displayName, int latency,
      int gameMode, @Nullable ChatSession chatSession, boolean listed) {
    return new KeyedLinkTabListEntry(this, profile, displayName, latency, gameMode,
    ...
    for (LegacyPlayerListItemPacket.Item item : packet.getItems()) {
      ...
      switch (packet.getAction()) {
        case LegacyPlayerListItemPacket.ADD_PLAYER: {
          ...
          entries.putIfAbsent(item.getUuid(), (KeyedLinkTabListEntry) TabListEntry.builder()
              .tabList(this)
        ...
        case LegacyPlayerListItemPacket.UPDATE_DISPLAY_NAME: {
          KeyedLinkTabListEntry entry = entries.get(uuid);
          if (entry != null) {

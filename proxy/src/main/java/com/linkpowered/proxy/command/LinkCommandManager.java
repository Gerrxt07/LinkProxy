...
package com.linkpowered.proxy.command;
...
import com.linkpowered.api.command.Command;
import com.linkpowered.api.command.CommandManager;
import com.linkpowered.api.command.CommandMeta;
import com.linkpowered.api.command.CommandResult;
import com.linkpowered.api.command.CommandSource;
import com.linkpowered.api.event.command.CommandExecuteEvent;
import com.linkpowered.api.event.command.PostCommandInvocationEvent;
import com.linkpowered.api.plugin.PluginManager;
import com.linkpowered.proxy.command.brigadier.LinkBrigadierCommandWrapper;
import com.linkpowered.proxy.command.registrar.BrigadierCommandRegistrar;
import com.linkpowered.proxy.command.registrar.CommandRegistrar;
import com.linkpowered.proxy.command.registrar.RawCommandRegistrar;
import com.linkpowered.proxy.command.registrar.SimpleCommandRegistrar;
...
/**
 * Implements Link's command handler.
 */
public class LinkCommandManager implements CommandManager {
  ...
  private final Map<SuggestionRequestKey, CompletableFuture<Suggestions>> inFlightSuggestions;
  private final AtomicLong commandGraphVersion = new AtomicLong();
   ...
   */
  public LinkCommandManager(final LinkEventManager eventManager,
    ...
    this.inFlightSuggestions = new ConcurrentHashMap<>();
  }
  ...
  private boolean executeImmediately0(final CommandSource source, final ParseResults<CommandSource> parsed) {
    ...
    try {
      ...
      if (isSyntaxError) {
        ...
        result = com.linkpowered.api.command.CommandResult.SYNTAX_ERROR;
  ...
  public CompletableFuture<Suggestions> offerBrigadierSuggestions(
    ...
    final String normalizedInput = LinkCommands.normalizeInput(cmdLine, false);
    final SuggestionRequestKey key = new SuggestionRequestKey(
        source, normalizedInput, commandGraphVersion.get());
    try {
      return this.inFlightSuggestions.computeIfAbsent(key, this::createSuggestionFuture);
    } catch (final Throwable e) {
  ...
  private CompletableFuture<Suggestions> createSuggestionFuture(final SuggestionRequestKey key) {
    final CompletableFuture<Suggestions> future =
        suggestionsProvider.provideSuggestions(key.input, key.source);
    future.whenComplete((suggestions, throwable) -> this.inFlightSuggestions.remove(key, future));
    return future;
  }

  private void invalidateSuggestionState() {
    commandGraphVersion.incrementAndGet();
  ...
  private ParseResults<CommandSource> parse(final String input, final CommandSource source) {
    final String normalizedInput = LinkCommands.normalizeInput(input, true);
    lock.readLock().lock();
    try {
  ...
  private Executor getAsyncExecutor(ParseResults<CommandSource> parse) {
    Object registrant;
    if (parse.getContext().getCommand() instanceof LinkBrigadierCommandWrapper vbcw) {
      registrant = vbcw.registrant() == null ? LinkVirtualPlugin.INSTANCE : vbcw.registrant();
    } else {
      registrant = LinkVirtualPlugin.INSTANCE;
    }
  ...
  private Executor figureAsyncExecutorForParsing() {
    ...
    if (thread instanceof FastThreadLocalThread) {
      ...
      return pluginManager.ensurePluginContainer(LinkVirtualPlugin.INSTANCE).getExecutorService();
    } else {
  ...
  private static final class SuggestionRequestKey {
    ...
    private SuggestionRequestKey(CommandSource source, String input, long graphVersion) {
      this.source = source;
      this.input = input;
      this.graphVersion = graphVersion;
      int result = System.identityHashCode(source);
      result = 31 * result + input.hashCode();
      result = 31 * result + Long.hashCode(graphVersion);
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof SuggestionRequestKey that)) {
        return false;
      }

...
final class SuggestionsProvider<S> {
  ...
  private CompletableFuture<Suggestions> provideArgumentsSuggestions(
    ...
    final LinkArgumentCommandNode<S, ?> argsNode = LinkCommands.getArgumentsNode(alias);
    if (argsNode == null) {
  ...
  private CompletableFuture<Suggestions> getArgumentsNodeSuggestions(
      final LinkArgumentCommandNode<S, ?> node, final StringReader reader,
   ...
   * @see LinkCommandMeta#copyHints(CommandMeta) for the conditions under which the returned
  ...
  private ParseResults<S> parseHints(final CommandNode<S> node, final StringReader originalReader,
    ...
    for (final CommandNode<S> child : node.getRelevantNodes(originalReader)) {
      if (LinkCommands.isArgumentsNode(child)) {
        continue;
      }
      final CommandContextBuilder<S> context = contextSoFar.copy();

/*
 * Copyright (C) 2021-2023 Link Contributors
 *
...
import com.linkpowered.api.command.InvocableCommand;
import com.linkpowered.proxy.command.brigadier.LinkArgumentCommandNode;
...
public final class LinkCommands {
  ...
  /**
   * Walks the command node tree and wraps all {@link Command} instances in a {@link LinkBrigadierCommandWrapper},
  ...
  public static CommandNode<CommandSource> wrap(final CommandNode<CommandSource> delegate,
    ...
    if (maybeCommand != null && !(maybeCommand instanceof LinkBrigadierCommandWrapper)) {
      maybeCommand = LinkBrigadierCommandWrapper.wrap(delegate.getCommand(), registrant);
    }
    ...
    return switch (delegate) {
      ...
      case LinkArgumentCommandNode<CommandSource, ?> vacn -> vacn.withCommand(maybeCommand)
   ...
   */
  static <S> @Nullable LinkArgumentCommandNode<S, ?> getArgumentsNode(
      final LiteralCommandNode<S> alias) {
    final CommandNode<S> node = alias.getChild(ARGS_NODE_NAME);
    if (node instanceof LinkArgumentCommandNode) {
      return (LinkArgumentCommandNode<S, ?>) node;
    }
    return null;
  }
  ...
  public static boolean isArgumentsNode(final CommandNode<?> node) {
    return node instanceof LinkArgumentCommandNode && node.getName().equals(ARGS_NODE_NAME);
  }

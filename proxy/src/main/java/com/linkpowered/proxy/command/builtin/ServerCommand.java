...
/**
 * Implements Link's {@code /server} command.
 */
public final class ServerCommand {
  ...
  public static BrigadierCommand create(final ProxyServer server) {
    final LiteralCommandNode<CommandSource> node = BrigadierCommand
        ...
        .requires(src -> src instanceof Player
                && src.getPermissionValue("link.command.server") != Tristate.FALSE)
        .executes(ctx -> {
  ...
  private static void outputServerInformation(final Player executor,
    ...
    executor.sendMessage(Component.translatable(
        "link.command.server-current-server",
    ...
    if (servers.size() > MAX_SERVERS_TO_LIST) {
      executor.sendMessage(Component.translatable(
          "link.command.server-too-many", NamedTextColor.RED));
      return;
    }

    // Assemble the list of servers as components
    final TextComponent.Builder serverListBuilder = Component.text()
        .append(Component.translatable("link.command.server-available",
            NamedTextColor.YELLOW))
        .appendSpace();
  ...
  private static TextComponent formatServerComponent(final String currentPlayerServer,
    ...
    if (connectedPlayers == 1) {
      playersTextComponent.key("link.command.server-tooltip-player-online");
    } else {
      playersTextComponent.key("link.command.server-tooltip-players-online");
    }
    ...
    if (serverInfo.getName().equals(currentPlayerServer)) {
      serverTextComponent.color(NamedTextColor.GREEN)
          .hoverEvent(
              showText(
                  Component.translatable("link.command.server-tooltip-current-server")
      ...
      serverTextComponent.color(NamedTextColor.GRAY)
          ...
          .hoverEvent(
              showText(
                  Component.translatable("link.command.server-tooltip-offer-connect-server")
                      .append(Component.newline())
                      .append(playersTextComponent))
          );
    }

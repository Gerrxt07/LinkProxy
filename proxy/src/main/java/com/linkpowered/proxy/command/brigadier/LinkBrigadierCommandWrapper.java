 ...
 */
public class LinkBrigadierCommandWrapper implements Command<CommandSource> {

  private final Command<CommandSource> delegate;
  private final Object registrant;

  private LinkBrigadierCommandWrapper(Command<CommandSource> delegate, Object registrant) {
    this.delegate = delegate;
    this.registrant = registrant;
  }

  /**
   * Transforms the given command into a {@code LinkBrigadierCommandWrapper} if the registrant
  ...
  public static Command<CommandSource> wrap(Command<CommandSource> delegate, @Nullable Object registrant) {
    ...
    if (delegate instanceof LinkBrigadierCommandWrapper) {
      // already wrapped
      return delegate;
    }
    return new LinkBrigadierCommandWrapper(delegate, registrant);
  }

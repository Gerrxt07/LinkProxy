...
/**
 * Implements the Link default {@code /glist} command.
 */
public class GlistCommand {
  ...
  public void register() {
    final LiteralArgumentBuilder<CommandSource> rootNode = BrigadierCommand
        ...
        .requires(source ->
            source.getPermissionValue("link.command.glist") == Tristate.TRUE)
  ...
  private int totalCount(final CommandContext<CommandSource> context) {
    ...
    source.sendMessage(
        Component.translatable("link.command.glist-view-all", NamedTextColor.YELLOW));
    return 1;
  }
  ...
  private void sendTotalProxyCount(CommandSource target) {
    ...
    final TranslatableComponent.Builder msg = Component.translatable()
            .key(online == 1
                  ? "link.command.glist-player-singular"
                  : "link.command.glist-player-plural"

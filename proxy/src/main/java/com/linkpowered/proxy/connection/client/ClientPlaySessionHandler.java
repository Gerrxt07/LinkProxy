...
import static com.linkpowered.proxy.protocol.util.PluginMessageUtil.constructChannelsPacket;

import com.google.common.collect.ImmutableList;
import com.linkpowered.api.event.connection.PluginMessageEvent;
import com.linkpowered.api.event.player.CookieReceiveEvent;
import com.linkpowered.api.event.player.PlayerChannelRegisterEvent;
import com.linkpowered.api.event.player.PlayerChannelUnregisterEvent;
import com.linkpowered.api.event.player.PlayerClientBrandEvent;
import com.linkpowered.api.event.player.TabCompleteEvent;
...
import com.linkpowered.proxy.connection.backend.BackendConnectionPhases;
...
import com.linkpowered.proxy.connection.player.resourcepack.ResourcePackResponseBundle;
...
import com.linkpowered.proxy.protocol.packet.TabCompleteResponsePacket.Offer;
...
import com.linkpowered.proxy.protocol.packet.chat.ChatTimeKeeper;
import com.linkpowered.proxy.protocol.packet.chat.CommandHandler;
import com.linkpowered.proxy.protocol.packet.chat.ComponentHolder;
import com.linkpowered.proxy.protocol.packet.chat.keyed.KeyedChatHandler;
import com.linkpowered.proxy.protocol.packet.chat.keyed.KeyedCommandHandler;
...
import com.linkpowered.proxy.protocol.packet.chat.legacy.LegacyChatHandler;
import com.linkpowered.proxy.protocol.packet.chat.legacy.LegacyChatPacket;
import com.linkpowered.proxy.protocol.packet.chat.legacy.LegacyCommandHandler;
import com.linkpowered.proxy.protocol.packet.chat.session.SessionChatHandler;
import com.linkpowered.proxy.protocol.packet.chat.session.SessionCommandHandler;
...
import com.linkpowered.proxy.protocol.util.PluginMessageUtil;
import com.linkpowered.proxy.util.CharacterUtil;
...
public class ClientPlaySessionHandler implements MinecraftSessionHandler {
  private static final boolean BACKPRESSURE_LOG =
      Boolean.getBoolean("link.log-server-backpressure");
  ...
  private static final long MAX_QUEUED_LOGIN_PLUGIN_MESSAGE_BYTES =
      Long.getLong("link.max-queued-login-plugin-message-bytes", 4L * 1024 * 1024);
  private static final int MAX_QUEUED_LOGIN_PLUGIN_MESSAGES =
      Integer.getInteger("link.max-queued-login-plugin-messages", 1024);
   ...
   */
  public ClientPlaySessionHandler(LinkServer server, ConnectedPlayer player) {
    this.player = player;
    this.server = server;
  ...
  private boolean validateChat(String message) {
    if (CharacterUtil.containsIllegalCharacters(message)) {
      player.disconnect(
          Component.translatable("link.error.illegal-chat-characters", NamedTextColor.RED));
      return false;
    }
    return true;
  }
  ...
  private boolean enqueueLoginPluginMessage(PluginMessagePacket packet) {
    ...
    if (newBytes > MAX_QUEUED_LOGIN_PLUGIN_MESSAGE_BYTES
      ...
      player.disconnect(Component.translatable("link.error.plugin-message-overflow"));
      return false;
    }
  ...
  public boolean handle(PluginMessagePacket packet) {
    ...
    LinkServerConnection serverConn =
    ...
    if (serverConn != null && backendConn != null) {
      if (backendConn.getState() != StateRegistry.PLAY) {
        ...
        if (serverConn.getPhase() == BackendConnectionPhases.IN_TRANSITION) {
          ...
          LinkServerConnection inFlight = player.getConnectionInFlight();
          if (inFlight != null) {
        ...
        if (!player.getPhase().handle(player, packet, serverConn)) {
          ...
          if (id == null) {
            ...
            if (!player.getPhase().consideredComplete() || !serverConn.getPhase()
              ...
              // but further aggravated by Link. To work around these issues, we will queue any
  ...
  public void exception(Throwable throwable) {
    player.disconnect(Component.translatable("link.error.player-connection-error", NamedTextColor.RED));
  ...
  public void writabilityChanged() {
    ...
    LinkServerConnection serverConn = player.getConnectedServer();
    if (serverConn != null) {
  ...
  public CompletableFuture<Void> doSwitch() {
    final LinkServerConnection existingConnection = player.getConnectedServer();
   ...
   * switching servers in Link.
   *
   * @param joinGame    the join game packet
   * @param destination the new server we are connecting to
   */
  public void handleBackendJoinGame(JoinGamePacket joinGame, LinkServerConnection destination) {
  ...
  private boolean handleCommandTabComplete(TabCompleteRequestPacket packet) {
    ...
    if (!server.getTabCompleteRateLimiter().attempt(player.getUniqueId())) {
      if (server.getConfiguration().isKickOnTabCompleteRateLimit()
        ...
        player.disconnect(Component.translatable("link.kick.tab-complete-rate-limit"));
      }

...
import static com.linkpowered.proxy.connection.forge.legacy.LegacyForgeConstants.HANDSHAKE_HOSTNAME_TOKEN;
 ...
 */
public class LinkServerConnection implements MinecraftConnectionAssociation, ServerConnection {

  private final LinkRegisteredServer registeredServer;
   ...
   * @param server           the Link proxy instance
   */
  public LinkServerConnection(LinkRegisteredServer registeredServer,
      @Nullable LinkRegisteredServer previousServer,
      ConnectedPlayer proxyPlayer, LinkServer server) {
   ...
   *
   * @return a {@link com.linkpowered.api.proxy.ConnectionRequestBuilder.Result}
    ...
    server.createBootstrap(proxyPlayer.getConnection().eventLoop())
        ...
        .addListener((ChannelFutureListener) future -> {
          if (future.isSuccess()) {
            ...
            connection.setAssociation(LinkServerConnection.this);
            ...
            if (!connection.setActiveSessionHandler(StateRegistry.HANDSHAKE)) {
              MinecraftSessionHandler handler =
                  new LoginSessionHandler(server, LinkServerConnection.this, result);
  ...
  @Override
  public LinkRegisteredServer getServer() {

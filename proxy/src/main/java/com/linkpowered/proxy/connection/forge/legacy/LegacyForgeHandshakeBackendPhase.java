...
package com.linkpowered.proxy.connection.forge.legacy;

import com.linkpowered.proxy.connection.ConnectionTypes;
import com.linkpowered.proxy.connection.MinecraftConnection;
import com.linkpowered.proxy.connection.backend.BackendConnectionPhase;
...
public enum LegacyForgeHandshakeBackendPhase implements BackendConnectionPhase {
  ...
  HELLO(LegacyForgeConstants.MOD_LIST_DISCRIMINATOR) {
    ...
    @Override
    void onTransitionToNewPhase(LinkServerConnection connection) {
   ...
   *                          moved onto a new phase, and as such, Link should do so too
  ...
  @Override
  public final boolean handle(LinkServerConnection serverConnection,
  ...
  @Override
  public void onDepartForNewServer(LinkServerConnection serverConnection,
      ConnectedPlayer player) {
   ...
   *
   * @param serverConnection The server Link is connecting to
   * @param packet           The packet
   * @return The phase to transition to, which may be the same as before.
   */
  private LegacyForgeHandshakeBackendPhase getNewPhase(LinkServerConnection serverConnection,

...
import com.linkpowered.api.util.UuidUtils;
...
import com.linkpowered.proxy.protocol.util.ByteBufDataInput;
...
public class BungeeCordMessageResponder {
  ...
  private final LinkServer proxy;
  private final ConnectedPlayer player;

  BungeeCordMessageResponder(LinkServer proxy, ConnectedPlayer player) {
    this.proxy = proxy;
    this.player = player;
  }
  ...
  private void processForwardToServer(ByteBufDataInput in) {
    ...
    if (target.equals("ALL") || target.equals("ONLINE")) {
      try {
        for (RegisteredServer rs : proxy.getAllServers()) {
          if (!rs.getServerInfo().equals(currentUserServer)) {
            ((LinkRegisteredServer) rs).sendPluginMessage(LEGACY_CHANNEL,
      ...
      if (server.isPresent()) {
        ((LinkRegisteredServer) server.get()).sendPluginMessage(LEGACY_CHANNEL, toForward);
      } else {
        toForward.release();
      }
    }

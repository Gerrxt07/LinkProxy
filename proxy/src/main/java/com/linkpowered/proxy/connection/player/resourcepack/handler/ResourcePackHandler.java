...
package com.linkpowered.proxy.connection.player.resourcepack.handler;
...
public abstract sealed class ResourcePackHandler
  ...
  protected final LinkServer server;

  protected ResourcePackHandler(final ConnectedPlayer player, final LinkServer server) {
    this.player = player;
    this.server = server;
  }
  ...
  public void queueResourcePack(final @NotNull ResourcePackRequest request) {
    for (final net.kyori.adventure.resource.ResourcePackInfo pack : request.packs()) {
      final ResourcePackInfo resourcePackInfo = LinkResourcePackInfo.fromAdventureRequest(request, pack);
   ...
   * no action is required in Link.</p>
   *
   * <li><b>INVALID_URL</b>
  ...
  protected boolean handleResponseResult(
    ...
    // If Link, through a plugin, has sent a resource pack to the client,
    ...
    if (!handled) {
      final LinkServerConnection connectionInFlight = player.getConnectionInFlight();

...
import static com.linkpowered.proxy.connection.backend.BungeeCordMessageResponder.getBungeeCordChannel;

import com.google.common.base.Preconditions;
import com.google.common.collect.ImmutableList;
import com.linkpowered.api.command.CommandSource;
import com.linkpowered.api.event.command.PlayerAvailableCommandsEvent;
...
import com.linkpowered.api.event.player.PlayerResourcePackStatusEvent;
import com.linkpowered.api.event.player.ServerResourcePackRemoveEvent;
import com.linkpowered.api.event.player.ServerResourcePackSendEvent;
import com.linkpowered.api.event.proxy.ProxyPingEvent;
...
import com.linkpowered.proxy.command.CommandGraphInjector;
...
import com.linkpowered.proxy.protocol.util.DeferredByteBufHolder;
...
public class BackendPlaySessionHandler implements MinecraftSessionHandler {
  ...
  private static final int MAXIMUM_PACKETS_TO_FLUSH =
      Integer.getInteger("link.max-packets-per-flush", 8192);
  ...
  BackendPlaySessionHandler(LinkServer server, LinkServerConnection serverConn) {
    this.server = server;
  ...
  public boolean handle(final ResourcePackRequestPacket packet) {
    final ResourcePackInfo.Builder builder = new LinkResourcePackInfo.BuilderImpl(
  ...
  public boolean handle(AvailableCommandsPacket commands) {
    ...
    if (server.getConfiguration().isAnnounceProxyCommands()) {
      ...
      if (this.playerConnection.getProtocolVersion().lessThan(ProtocolVersion.MINECRAFT_1_21_6)) {
        rootNode.removeChildByName("link:callback");
      }
    }

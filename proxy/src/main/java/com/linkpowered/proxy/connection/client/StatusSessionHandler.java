...
public class StatusSessionHandler implements MinecraftSessionHandler {
  ...
  private final LinkInboundConnection inbound;
  private boolean pingReceived = false;

  StatusSessionHandler(LinkServer server, LinkInboundConnection inbound) {
    this.server = server;
        ...
        .thenAcceptAsync(
            (event) -> {
              if (event.getResult().isAllowed()) {
                ...
                LinkServer.getPingGsonInstance(connection.getProtocolVersion())

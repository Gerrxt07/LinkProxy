...
import static com.linkpowered.proxy.network.Connections.CIPHER_DECODER;
import static com.linkpowered.proxy.network.Connections.CIPHER_ENCODER;
import static com.linkpowered.proxy.network.Connections.COMPRESSION_DECODER;
import static com.linkpowered.proxy.network.Connections.COMPRESSION_ENCODER;
...
import com.linkpowered.proxy.connection.client.InitialLoginSessionHandler;
import com.linkpowered.proxy.connection.client.StatusSessionHandler;
import com.linkpowered.proxy.network.Connections;
import com.linkpowered.proxy.network.limiter.SimpleBytesPerSecondLimiter;
import com.linkpowered.proxy.protocol.LinkConnectionEvent;
...
import com.linkpowered.proxy.protocol.netty.MinecraftCipherDecoder;
import com.linkpowered.proxy.protocol.netty.MinecraftCipherEncoder;
import com.linkpowered.proxy.protocol.netty.MinecraftCompressDecoder;
import com.linkpowered.proxy.protocol.netty.MinecraftCompressorAndLengthEncoder;
...
public class MinecraftConnection extends ChannelInboundHandlerAdapter {
  ...
  public final LinkServer server;
   ...
   * @param server  the Link instance
   */
  public MinecraftConnection(Channel channel, LinkServer server) {
  ...
  public void setProtocolVersion(ProtocolVersion protocolVersion) {
    ...
    if (changed) {
      channel.pipeline().fireUserEventTriggered(LinkConnectionEvent.PROTOCOL_VERSION_CHANGED);
    }
  }
  ...
  public void setCompressionThreshold(int threshold) {
    ...
    if (threshold == -1) {
      ...
      if (removedDecoder != null && removedEncoder != null) {
        ...
        channel.pipeline().fireUserEventTriggered(LinkConnectionEvent.COMPRESSION_DISABLED);
      }
    } else {
      ...
      if (decoder != null && encoder != null) {
        ...
        LinkCompressor compressor = Natives.compress.get().create(level);
        ...
        channel.pipeline().fireUserEventTriggered(LinkConnectionEvent.COMPRESSION_ENABLED);
      }
    }
  }
  ...
  public void enableEncryption(byte[] secret) throws GeneralSecurityException {
    ...
    LinkCipherFactory factory = Natives.cipher.get();
    LinkCipher decryptionCipher = factory.forDecryption(key);
    LinkCipher encryptionCipher = factory.forEncryption(key);
    channel.pipeline()
    ...
    channel.pipeline().fireUserEventTriggered(LinkConnectionEvent.ENCRYPTION_ENABLED);
  }

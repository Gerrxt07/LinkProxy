...
package com.linkpowered.proxy.protocol.packet;

import static com.linkpowered.proxy.protocol.util.PluginMessageUtil.transformLegacyToModernChannel;
...
import com.linkpowered.proxy.protocol.ProtocolUtils.Direction;
...
public class PluginMessagePacket extends DeferredByteBufHolder implements MinecraftPacket {
  ...
  private static int getPayloadLimit(Direction direction) {
    if (System.getProperty("link.max-plugin-message-payload-size") != null) {
      return Integer.getInteger("link.max-plugin-message-payload-size");
    }
    if (direction == Direction.SERVERBOUND) {
      return Integer.getInteger("link.max-plugin-message-payload-size.serverbound", 32767);
    } else {
      // This is the vanilla expected limit, a payload this large feels like a nightmare given the trust
      // we give to servers...
      return Integer.getInteger("link.max-plugin-message-payload-size.clientbound", 1048576);
    }
  }

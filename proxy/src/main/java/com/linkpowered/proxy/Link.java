...
import com.linkpowered.proxy.util.LinkProperties;
...
public class Link {
  ...
  static {
    ...
    logger = LogManager.getLogger(Link.class);
    ...
    // If Link's natives are being extracted to a different temporary directory, make sure the
    // Netty natives are extracted there as well
    if (LinkProperties.hasProperty("link.natives-tmpdir")) {
      System.setProperty("io.netty.native.workdir", System.getProperty("link.natives-tmpdir"));
    }

    // Disable the resource leak detector by default as it reduces performance. Allow the user to
    // override this if desired.
    if (!LinkProperties.hasProperty("io.netty.leakDetection.level")) {
      ResourceLeakDetector.setLevel(Level.DISABLED);
    }
  }

  /**
   * Main method that the JVM will call when {@code java -jar link.jar} is executed.
   *
  ...
  public static void main(String... args) {
    ...
    LinkServer server = new LinkServer(options);
    server.start();

...
for variant in "${BASE_DOCKERFILE_VARIANTS[@]}"; do
  ...
  docker build -t link-native-build:$variant $docker_platforms -f build-support/$variant.Dockerfile .
done
...
for arch in "${ARCHS[@]}"; do
  for variant in "${BASE_DOCKERFILE_VARIANTS[@]}"; do
    ...
    docker run --rm -v "$(pwd)":/app --platform linux/${arch} link-native-build:$variant /bin/bash -c "cd /app && ./build-support/compile-linux-crypto.sh"
  done

  for variant in "${COMPRESSION_VARIANTS[@]}"; do
    echo "Building native compression for $arch on $variant..."
    docker run --rm -v "$(pwd)":/app --platform linux/${arch} link-native-build:$variant /bin/bash -c "cd /app && ./build-support/compile-linux-compress.sh"
  done
done
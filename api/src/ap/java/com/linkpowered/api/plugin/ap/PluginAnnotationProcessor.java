...
package com.linkpowered.api.plugin.ap;

import com.google.auto.service.AutoService;
import com.google.gson.Gson;
import com.linkpowered.api.plugin.Dependency;
import com.linkpowered.api.plugin.Plugin;
...
/**
 * Annotation processor for Link.
 */
@AutoService(Processor.class)
@SupportedAnnotationTypes({"com.linkpowered.api.plugin.Plugin"})
    ...
    for (Element element : roundEnv.getElementsAnnotatedWith(Plugin.class)) {
      ...
      if (pluginClassFound != null) {
        if (!pluginClassFound.equals(qualifiedName.toString()) && !warnedAboutMultiplePlugins) {
          environment.getMessager()
              .printMessage(Diagnostic.Kind.WARNING, "Link does not yet currently support "
      ...
      // All good, generate the link-plugin.json.
      ...
      try {
        FileObject object = environment.getFiler()
            .createResource(StandardLocation.CLASS_OUTPUT, "", "link-plugin.json");

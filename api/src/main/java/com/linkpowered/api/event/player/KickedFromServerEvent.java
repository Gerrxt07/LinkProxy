...
/**
 * Fired when a player is kicked from a server. You may either allow Link to kick the player
 * (with an optional reason override) or redirect the player to a separate server. By default,
 * Link will notify the user (if they are already connected to a server) or disconnect them
 * (if they are not on a server and no other servers are available). Link will wait on this
